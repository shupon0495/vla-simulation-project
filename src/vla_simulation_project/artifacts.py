...
def read_json(path: Path) -> dict:
    ...
    if not isinstance(value, dict): raise ValueError(f"manifest must contain an object: {path}")
    return value
...
def validate_policy_directory(path: Path) -> None:
    ...
    for pattern in ("policy_preprocessor*.safetensors", "policy_postprocessor*.safetensors"):
        if not any(p.is_file() and p.stat().st_size for p in path.glob(pattern)): missing.append(pattern)
    if missing: raise FileNotFoundError(f"incomplete merged policy at {path}; missing/non-empty required files: {missing}")

def archive_model(model: Path, archive: Path) -> None:
    validate_policy_directory(model)
    with tarfile.open(archive, "w:gz") as output: output.add(model, arcname=model.name)
...
def result_rows(run_id: str, suite: str, info: dict) -> list[dict]:
    ...
    for task in info.get("per_task", []):
        ...
        if not successes: raise ValueError(f"no episode success values for {suite} task {task.get('task_id')}")
        count, succeeded = len(successes), sum(successes)
        total_trials += count; total_success += succeeded
    ...
    if not rows: raise ValueError(f"no task results in {suite} eval_info.json")
...
def write_parameter_csv(path: Path, run_id: str, values: dict) -> None:
    with path.open("w", newline="", encoding="utf-8") as output:
        writer = csv.DictWriter(output, fieldnames=("run_id", "parameter", "value")); writer.writeheader()
...
def validate_final_artifacts(run: Path, run_id: str, suites: tuple[str, ...]) -> None:
    required = [run / f"{run_id}_model.tar.gz", run / f"{run_id}_parameters.csv", run / f"{run_id}_results.csv",
                *(run / f"{run_id}_{name}.mp4" for name in ("spatial", "object", "goal", "libero10")),
                *(run / "manifests" / name for name in ("run.json", "preprocess.json", "train.json", "test.json"))]
    missing = [str(path) for path in required if not path.is_file() or not path.stat().st_size]
    if missing: raise FileNotFoundError(f"run artifact contract is incomplete: {missing}")
    ...
    with (run / f"{run_id}_results.csv").open(newline="", encoding="utf-8") as source: rows = list(csv.DictReader(source))
    if any(row["run_id"] != run_id for row in rows): raise ValueError("RUN_ID mismatch in results CSV")
    present = {row["suite"] for row in rows if row["level"] == "suite"}
    if present != set(suites): raise ValueError(f"results CSV suite rows mismatch: {sorted(present)}")
    for row in rows:
        trials, successes, rate = int(row["n_trials"]), int(row["n_success"]), float(row["success_rate"])
        if trials < 1 or not 0 <= successes <= trials or abs(rate - successes / trials) > 1e-12: raise ValueError(f"invalid results row: {row}")
