...
def normalize_task_name(value: str) -> str:
    ...
    return re.sub('\\s+', ' ', re.sub('[^a-z0-9 ]+', ' ', value)).strip()

def task_name_from_cell(value: object) -> str:
    if isinstance(value, str):
        return value
    try:
        return str(value[0]) if len(value) else str(value)
    except TypeError:
        return str(value)

def choose_evenly_spaced(episode_indices: list[int], count: int) -> list[int]:
    if count < 1 or len(episode_indices) < count:
        raise ValueError(f'task has {len(episode_indices)} episodes but {count} are required')
    if count == 1:
...
def select_spatial_episodes(tasks: list[object], per_task: int=5) -> tuple[list[int], dict[str, list[int]]]:
    ...
    for index, cell in enumerate(tasks):
        grouped[task_name_from_cell(cell)].append(index)
    available = {normalize_task_name(name): name for name in grouped}
    selected = {}
    for expected in SPATIAL_TASK_NAMES:
        actual = available.get(normalize_task_name(expected))
        if actual is None:
            raise RuntimeError(f'Spatial task not found in local dataset metadata: {expected}')
    ...
    if len(indices) != len(SPATIAL_TASK_NAMES) * per_task:
        raise RuntimeError('episode selection produced an unexpected count')
...
def load_episode_tasks(dataset: Path) -> list[object]:
    ...
    if jsonl.is_file():
        ...
        for line in jsonl.read_text(encoding='utf-8').splitlines():
            row = json.loads(line)
            tasks.append(row.get('tasks', row.get('task')))
        return tasks
