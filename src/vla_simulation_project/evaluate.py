...
def build_eval_command(policy: Path, output: Path, suite: str, config: ExperimentConfig) -> list[str]:
    return ["lerobot-eval", f"--policy.path={policy}", "--policy.device=cuda", "--policy.use_amp=false", "--env.type=libero",
        "--env.is_libero_plus=true", f"--env.task={suite}", "--env.task_ids=" + json.dumps(list(config.task_ids), separators=(",", ":")),
        "--env.camera_name_mapping=" + json.dumps(CAMERAS, separators=(",", ":")), "--env.observation_height=256",
        "--env.observation_width=256", "--env.control_mode=relative", "--env.max_parallel_tasks=1", "--eval.batch_size=1",
        ...
        if int(task.get("task_id", -1)) == task_id:
            ...
            if paths:
                ...
                for possible in (candidate, output / candidate):
                    if possible.is_file() and possible.stat().st_size: return possible
    directory = output / "videos" / f"{info.get('task_group', '')}_{task_id}"
    candidates = sorted(directory.rglob("*.mp4")) if directory.is_dir() else []
    if not candidates: raise FileNotFoundError(f"test: evaluation did not produce a video for task {task_id} under {output}")
    return candidates[0]
...
def evaluate() -> None:
    ...
    for suite in SUITES:
        output = run / "eval" / suite; command = build_eval_command(policy, output, suite, config); commands[suite] = command
        run_command("test", command, cwd=paths.project, env=env)
        info = read_json(output / "eval_info.json"); rows.extend(result_rows(paths.run_id(), suite, info))
        target = run / f"{paths.run_id()}_{names[suite]}.mp4"; shutil.copy2(_task_video(info, config.video_task_id, output), target); videos[suite] = str(target)
    results = run / f"{paths.run_id()}_results.csv"; write_results_csv(results, rows)
    write_json(run / "manifests/test.json", {"run_id": paths.run_id(), "stage": "test", "slurm_job_id": os.environ.get("SLURM_JOB_ID"),
        "evaluation_started_at": started, "evaluation_finished_at": datetime.now(timezone.utc).isoformat(), "suites": list(SUITES),
        "task_ids": list(config.task_ids), "episodes_per_task": config.episodes_per_task, "results_csv_path": str(results),
        "video_paths": videos, "commands": commands})
