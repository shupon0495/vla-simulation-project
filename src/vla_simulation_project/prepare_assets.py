...
from .config import (BASE_MODEL_REPO, BASE_MODEL_REVISION, DATASET_REPO, DATASET_REVISION,
                     LIBERO_ASSETS_REPO, LIBERO_SOURCE_REPO, LIBERO_SOURCE_REVISION, VLM_REPO)
...
def _valid_libero_source(source: Path, revision: str) -> bool:
    ...
    return (
        ...
        and (resources / "bddl_files").is_dir()
        and (resources / "init_files").is_dir()
    )
...
def _prepare_libero_source(paths: ProjectPaths, assets: Path) -> Path:
    ...
    try:
        ...
        if actual_revision != LIBERO_SOURCE_REVISION:
            raise RuntimeError(
                f"LIBERO-plus source revision mismatch at {staging}: expected {LIBERO_SOURCE_REVISION}, got {actual_revision}"
            )
        if not _valid_libero_source(staging, LIBERO_SOURCE_REVISION):
            raise FileNotFoundError(
                f"LIBERO-plus source resources missing under {staging}; expected libero/libero/bddl_files and libero/libero/init_files"
            )
...
def validate_assets(paths: ProjectPaths, *, repair_tokenizer: bool = False) -> dict:
    ...
    source_item = lock.get("libero_source", {}); source = paths.project / str(source_item.get("local_path", ""))
    if (source_item.get("repo") != LIBERO_SOURCE_REPO or source_item.get("revision") != LIBERO_SOURCE_REVISION
            or not _valid_libero_source(source, LIBERO_SOURCE_REVISION)):
        raise FileNotFoundError(
            f"missing/invalid LIBERO-plus benchmark resources for {LIBERO_SOURCE_REPO}@{LIBERO_SOURCE_REVISION} "
            f"at {source}; rerun prepare-assets on the Internet-connected login node"
        )
    assets_link = _resource_root(source) / "assets"
    if not assets_link.is_symlink() or assets_link.resolve() != local.resolve():
        raise FileNotFoundError(
            f"LIBERO-plus source assets link is missing/invalid at {assets_link}; rerun prepare-assets on the Internet-connected login node"
        )
    return lock
...
def prepare_assets() -> None:
    ...
    lock = {"base_model": {"repo": BASE_MODEL_REPO, "revision": BASE_MODEL_REVISION, "local_path": _relative(paths, base)},
            ...
            "libero_source": {"repo": LIBERO_SOURCE_REPO, "revision": LIBERO_SOURCE_REVISION, "local_path": _relative(paths, source)}}
